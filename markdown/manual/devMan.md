...
本書では、「樹木管理ダッシュボード」の利用環境構築手順について記載しています。従来、地方公共団体や民間企業が紙やPDFで管理しており、形式や所在が統一されていなかった緑地関連データをデータベースで一元的に管理する機能を提供します。
...
| オープンソースRDBMS | [PostgreSQL](https://github.com/postgres/postgres) | 16.2 | 樹木台帳データ等を格納するリレーショナルデータベース |
...
本システムで利用する下記のソフトウェア・サービスを準備します。なお、「樹木データを活用した温熱環境シミュレータの開発」における他の3つのアプリケーションで構築されたPostGISやFirebase等の環境がある場合は、当該環境をご利用いただくことも可能です。
...
樹木台帳データを PostgreSQL に格納します。スキーマは、技術検証レポートにてご確認ください。

（2）3D都市モデルデータの登録

3D都市モデル（3DTiles、建築物モデル・植生モデル等）を Amazon S3 に格納します。3D都市モデル（3DTiles） は、[G空間情報センター](https://front.geospatial.jp/)からダウンロードしてください。植生モデル（LOD1,2）のCityGML、3DTilesは、本リポジトリの[FMEワークスペース](https://github.com/Project-PLATEAU/green-dashboard/tree/main/workspaces)で作成することが可能です。

（3）ユーザの登録

Firebase コンソールまたはバックエンドAPIのユーザ管理機能から初期ユーザを登録します。

※各システムへのアクセス権はシステムごとに個別に設定・登録が必要です。
